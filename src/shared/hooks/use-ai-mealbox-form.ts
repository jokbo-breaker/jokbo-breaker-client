...
import type { FoodOption, MethodId } from '@/pages/recommend/constants/recommend';
...
export function useAiMealboxForm() {
  ...
  const toggleFood = useCallback((v: FoodOption) => {
    setFoods((prev) => (prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v]));
  }, []);
  ...
  const payload = useMemo(() => ({ foods, maxPrice, method }), [foods, maxPrice, method]);
